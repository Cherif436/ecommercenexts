...
export default function ProductScreen(props) {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      setLoading(false);
      enqueueSnackbar('Avis soumis avec succès', { variant: 'success' });
      fetchReviews();
    } catch (err) {
      setLoading(false);
  ...
  return (
    <Layout title={product.name} description={product.description}>
      ...
      <List>
        ...
        {reviews.length === 0 && <ListItem>Aucun avis</ListItem>}
        {reviews.map((review) => (
        ...
        <ListItem>
          {userInfo ? (
            <form onSubmit={submitHandler} className={classes.reviewForm}>
              <List>
                <ListItem>
                  <Typography variant="h2">Laissez votre avis</Typography>
                </ListItem>
                <ListItem>
                  <TextField
                    multiline
